...
export default function FertPump() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const sMin = await AsyncStorage.getItem("fp_min");
        const sMax = await AsyncStorage.getItem("fp_max");
    ...
    const fetchBlynkStatus = async () => {
      try {
        const res = await fetch(
          `https://blynk.cloud/external/api/get?token=${TOKEN}&v21&v22`
        );
        const data = await res.json();

        if (data.v22 !== undefined) setIsOn(data.v22 == 1);
        if (data.v21 !== undefined) setIsAuto(data.v21 == 1);
      } catch (err) {
  ...
  const applyValues = () => {
    if (minValue) {
      ...
      saveData("fp_min", minValue);
      fetch(
        `https://blynk.cloud/external/api/update?token=${TOKEN}&V24=${minValue}`
      ).catch(console.log);
    }

    if (maxValue) {
      setSavedMax(maxValue);
      saveData("fp_max", maxValue);
      fetch(
        `https://blynk.cloud/external/api/update?token=${TOKEN}&V25=${maxValue}`
      ).catch(console.log);
    }
  ...
  const updateOnOff = (val) => {
    setIsOn(val);
    fetch(
      `https://blynk.cloud/external/api/update?token=${TOKEN}&V22=${val ? 1 : 0}`
    ).catch(console.log);
  };
  ...
  const updateMode = (auto) => {
    setIsAuto(auto);
    fetch(
      `https://blynk.cloud/external/api/update?token=${TOKEN}&V21=${
        auto ? 1 : 0
      }`
    ).catch(console.log);
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        <Text style={styles.cfHeader}>Fertilizer Pump</Text>
